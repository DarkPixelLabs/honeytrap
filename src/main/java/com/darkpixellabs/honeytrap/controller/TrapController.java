...
import com.darkpixellabs.honeytrap.service.HitLoggingService; import jakarta.servlet.http.HttpServletRequest; import org.springframework.http.*; import org.springframework.web.bind.annotation.*; import java.util.*;
@RestController public class TrapController {
 private final HitLoggingService logger; private static final Set<String> SCANNER_MARKERS=Set.of(".php",".asp",".aspx","wp-","xmlrpc.php","phpmyadmin","cgi-bin","actuator","server-status",".env",".git/");
 ...
 @GetMapping(value="/.env",produces=MediaType.TEXT_PLAIN_VALUE)public String env(HttpServletRequest r){logger.log(r);return fakeEnv();}
 @GetMapping(value="/.git/config",produces=MediaType.TEXT_PLAIN_VALUE)public String gitConfig(HttpServletRequest r){logger.log(r);return fakeGit();}
 ...
 @GetMapping(value="/{*path}")public ResponseEntity<?>scannerCatchAll(HttpServletRequest r){String p=r.getRequestURI().toLowerCase(Locale.ROOT);if(p.equals("/.env")){logger.log(r);return ResponseEntity.ok().contentType(MediaType.TEXT_PLAIN).body(fakeEnv());}if(p.equals("/.git/config")){logger.log(r);return ResponseEntity.ok().contentType(MediaType.TEXT_PLAIN).body(fakeGit());}if(SCANNER_MARKERS.stream().anyMatch(p::contains))logger.log(r);return ResponseEntity.status(HttpStatus.NOT_FOUND).build();}
 private String fakeEnv(){return "# HONEYTRAP FAKE CONFIG\nAPP_NAME=honeytrap-demo\nDB_HOST=placeholder.invalid\nDB_PASSWORD=NOT_A_REAL_PASSWORD\nSECRET_KEY=DEMO_ONLY_NOT_SECRET\n";}
 private String fakeGit(){return "[core]\n\trepositoryformatversion = 0\n\tbare = false\n[remote \"origin\"]\n\turl = https://example.invalid/honeytrap-demo.git\n";}
